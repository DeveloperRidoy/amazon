...
function Cart () {
  ...
  const otherProducts = state.products.filter(product => cartState?.some(prod => prod.product._id !== product._id)).filter((product, i) => i <= 12)
  ...
  const updateCart = async e => {
    ...
    try {
      ...
      const res = await axios.patch(`${process.env.NEXT_PUBLIC_API || 'api'}/v1/users/update-me`, { cart: cartState }, { withCredentials: true });

      setState({ ...state, alert: { type: 'success', message: 'Cart updated' }, user: res.data.data.user });

      setLoading(false);
    } catch (error) {
      setState({ ...state, alert: { type: 'danger', message: error.response?.message || error.message || 'Network Error' } });
    }
  }
          ...
          <div className="row mx-0">
            <form className="col-lg-8 ml-lg-3 mb-5" onSubmit={updateCart}>
              ...
              <div className="d-sm-flex align-items-center justify-content-between mt-3">
                <div
                  className="input-group mb-3 mb-sm-0"
                  style={{ width: 250 }}
                >
                  <input
                    type="text"
            ...
            <div className="col border-left">
              ...
              {otherProducts.map((product) => (
                <Link href={`/shop/${product.category.name}/${product.slug}`} key={product._id}>
                  <a>
                    <img
