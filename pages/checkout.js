...
function Checkout () {
  ...
  const inputChange = e => setBillingData({ ...billingData, [e.target.name]: e.target.type === 'checkbox' ? e.target.checked : e.target.value });
  ...
  const initiateCheckout = async e => {
    try {
      ...
      const res = await axios.post(`${process.env.NEXT_PUBLIC_API || 'api'}/v1/create-checkout-session`, data , { withCredentials: true });

      // redirect to strip checkout page
      const result = await stripe.redirectToCheckout({ sessionId: res.data.data.sessionId });

      // show error message on error
      if (result.error) {
        return setState({ ...state, alert: { type: 'danger', message: result.error.message } });
      }
