...
function UserSettings (e) {
  ...
  const [filteredUsers, setFilteredUsers] = useState({
    ...
    const inputchange = e => setSearchData({ ...searchData, [e.target.name]: e.target.type === 'checkbox' ? e.target.checked : e.target.value });
    ...
    const searchUser = async (e) => {
      try {
        ...
        const res = await axios.post(`${process.env.NEXT_PUBLIC_API || 'api'}/v1/users/query`, data, { withCredentials: true });
        setLoading(false);
        const breadCrumbs = [];
        res.data.data.users.forEach((user, i) =>  i < res.data.data.users.length / filteredUsers.limit && breadCrumbs.push(i))
        setFilteredUsers({...filteredUsers, users: res.data.data.users, searched: true, breadCrumbs })
    ...
    return (
      <div style={{ position: "relative" }}>
        <form
          ...
          style={{ position: "sticky", top: 43, left: 0, zIndex: 2 }}
        >
        ...
        {filteredUsers.searched && filteredUsers.users.length > 0 ? (
          ...
          filteredUsers.users?.length === 0 && <h3>No users found</h3>
        )}
      </div>
    );
} 
