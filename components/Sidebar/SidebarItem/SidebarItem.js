...
function SidebarItem({
  ...
  const stateChange = async (link) => {
    ...
    if (chevron)
      return setSidebarSettings({
        ...sidebarSettings,
        showSubmenu: true,
        currentSubmenu: link.submenus,
      });

    // for links without submenu
    if (!link.logOut)
      return setSidebarSettings({ ...sidebarSettings, show: false });

    // For logout button
    try {
      setSidebarSettings({ ...sidebarSettings, loading: true });
      const res = await axios.get(
        `${process.env.NEXT_PUBLIC_API}/v1/users/logout`
      );
    ...
    <div className="pb-4">
      ...
      {links?.length > 0 &&
        links.map((link) => (
          <Link
            href={
              link.link === "#" ? Router.asPath : link.link || Router.asPath
            }
            key={link._id}
          >
            <a className={styles.menuItem} onClick={() => stateChange(link)}>
              <p>{link.text}</p>
